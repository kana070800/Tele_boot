...
void USART3_UART_Init(){
	...
	RCC_APB1ENR |= (0x1 << 18); // uart3 clk enable

	// GPIO mode
	GPIOD_MODER &= ~((0x3 << 8*2) | (0x3 << 9*2));
	...
	//uart setting
	USART3_CR1 |= (0x1 << 13); // uart3 enable
	USART3_CR1 &= ~(0x1 << 12); // 8bit length set

	USART3_CR2 &= ~(0x3 << 12); //stop bit 1 length
	// systemclock_config() 함수 사용시 uart에 42Mhz clk 인가
	USART3_BRR = 0x16D;
...
int main(void)
{
  ...
  STK_CTRL |= (1<<0);
  STK_CTRL &= ~(1<<2);
  STK_LOAD = 2099999; //0.1 초
  STK_VAL = 0;
  while (1)
  {
	while(!((STK_CTRL >> 16)& 0x1)){};
	if (++cnt >= 5){ // 0.5 초
...
int main(void)
{
  ...
  // AHB = 168MHZ로 초기화, 만일 내부에서 인터럽트가 걸리도록 하면 uart 이후 호출
  SystemClock_Config();
  STK_CTRL = 0;
  ...
  STK_CTRL |= (1<<1);
  STK_CTRL &= ~(1<<2);
  //enable을 마지막에 박아주는 것이 좋을 것
  //------------------------------
  /*
  int ms = 500, clk = 21000;
  STK_LOAD = ms*clk;
  */
  //-------------------------------
