...
//}
//
//void I2C1_read_ready(int sensor, uint8_t reg)
//{
//	while (h_I2C1.state != I2C_IDLE) {};
//	//set values
//	if (sensor == 3221)
...
//		    I2C1->CR1 |=  (1 << 11);      // POS = 1
//
//		    temp = I2C1->SR2;             // ADDR clear
//
//		    h_I2C1.state = I2C_READ_WAIT_BTF;
//
//		    return;
//		}
...
//	}
//
//	// btf condition = EV8_2, read
//	if (sr1 & I2C_SR1_BTF)
//	{
//		UART3_Printf("BTF %d\n", h_I2C1.state);
//		if(h_I2C1.state == I2C_SEND_REG8_2){ //generate stop
//			I2C1->CR1 |= (1 << 9);
//			h_I2C1.set_regs = 0;
//			h_I2C1.state = I2C_IDLE;
//		}
//		if(h_I2C1.state == I2C_READ_WAIT_BTF)
//		{
//		    /* STOP 먼저 */
//
...
//
//			// 🟢 [추가] 2바이트 수신 시작 시 마스터가 첫 바이트 응답으로 ACK를 주도록 설정
...
////		if(h_I2C1.state == I2C_READ_DATA){
////			I2C1->CR1 |= (1 << 9);
////			data_H = I2C1->DR; //read high data
////			data_L = I2C1->DR; //read low data
////			I2C1->CR1 &= ~(1 << 11);  // POS clear!!!!!!!!!!!!!!!!!!
////			I2C1->CR1 |= (1 << 10);   // ACK set!!!!!!!!!!!
////			h_I2C1.state = I2C_IDLE;
////			UART3_Printf("%d    %d", data_H, data_L);
////			UART3_Printf("BTF end !!!%d\n", h_I2C1.state);
////		}
//		return;
//	}
//
...
//	}
//	if (sr1 & I2C_SR1_RXNE){
//		UART3_Printf("sr1 %d,sr2 %d ,state = %d\n", I2C1->SR1, I2C1->SR2, h_I2C1.state );
//		switch (h_I2C1.state){
//			case I2C_READ_DATA_H:
//				data_H = I2C1->DR;
//				I2C1->CR1 &= ~(1 << 10); // NACK send
//				I2C1->CR1 |= (1 << 9);; // STOP 설정
//				h_I2C1.state = I2C_READ_DATA_L;
//				break;
//			case I2C_READ_DATA_L:
//				// 🟢 두 번째 바이트(LSB) 수신 완료
//				data_L = I2C1->DR;
//				// 모든 수신이 끝났으므로 IDLE 상태로 복귀
//				h_I2C1.state = I2C_IDLE;
//				//I2C1->CR2 &= ~(1 << 10);//++++++++++++++++
//				UART3_Printf("%d  %d\n",data_L, data_H);
//				break;
