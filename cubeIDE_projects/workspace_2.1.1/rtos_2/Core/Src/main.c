...
/* Definitions for num */
osThreadId_t numHandle;
const osThreadAttr_t num_attributes = {
  .name = "num",
  .stack_size = 128 * 4,
...
int main(void)
{
  ...
  /* Create the thread(s) */
  /* creation of num */
  //numHandle = osThreadNew(StartDefaultTask, NULL, &num_attributes);

  BaseType_t ret;

  for(int i = 0; i<10;++i){
	  ret = xTaskCreate(StartDefaultTask, "num_task", 200,(void *)(str + i), 10 - i,NULL);
	  if (ret != pdPASS){
		  UART3_Printf("%d error\n\r", i);
		  break;
	  }
	  UART3_Printf("task %d create\n\r", i);
  }
...
/**
  * @brief  Function implementing the num thread.
...
void StartDefaultTask(void *argument)
{
  /* USER CODE BEGIN 5 */
  /* Infinite loop */
	UART3_Printf("task %c start\n\r", *(char *)argument);
	int msec;
	char k = *(char *)argument;
	for(;;)
	{
		// event wait
		msec = rand()%401 + 100;
		...
		//lock
		// 1. taskENTER_CRITICAL();  // critical section
		//2. vTaskSuspendAll();  // suspend scaduler
		xSemaphoreTake( Uart_sem, portMAX_DELAY ); //mutex
		for (int i = 0; i < 30; ++i){
		UART3_Printf("%c%c", k, k-0x20);
		}
		UART3_Printf("\n");
		//lock
		//1. taskEXIT_CRITICAL();
